...
**`priceScaleId: ''` is a shared hidden overlay scale.** It often holds volume. Use a distinct `overlay:${string}` id for an independent hidden scale; sharing an id shares its range and baseline. Comparison handles select unoccupied scales automatically. Only the **right** scale expands for primitives, so a price line or custom primitive on a left or overlay scale can be clipped out of view.
