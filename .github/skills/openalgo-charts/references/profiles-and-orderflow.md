...
| `showPoc` / `pocColor` / `pocOutline` / `pocOutlineWidth` | `true` / `#f0a020` / *(unset)* / `1` | `showPoc` is a 2 px tick in the left margin; `pocOutline` takes a colour and rings the POC row itself with **one closed rectangle around the whole row**, not corner marks. `pocOutlineWidth` is media px and the ring is inset by half of it. |
| `buyColor` / `sellColor` | *(unset)* | Fall back to `theme.upColor` / `theme.downColor`. |
| `cellBaseColor` | *(unset)* | Opaque plate a cell is tinted **from**, in place of the pane background: off a dark pane the quiet end of the ladder is near black and a one-lot row is a hole. Also backs the flat delta column of `cells: 'deltaVolume'`. |
