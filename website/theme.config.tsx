...
const Logo = () => (
  <span style={{ display: 'flex', alignItems: 'center', gap: 8, fontWeight: 800, letterSpacing: '-0.02em', fontSize: '1.05rem' }}>
    <span
      aria-hidden
      style={{
        width: 24, height: 24, display: 'inline-block', flex: 'none',
        backgroundColor: 'currentColor', // black in light mode, white in dark mode
        WebkitMaskImage: 'url(/openalgo-charts/openalgo-mark.svg)',
        maskImage: 'url(/openalgo-charts/openalgo-mark.svg)',
...
const config = {
  ...
  footer: {
    content: (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 4, fontSize: '0.85rem' }}>
        <span>Apache-2.0 - Original, dependency-free code - 59 KB Brotli base engine</span>
  ...
  head: function Head() {
    ...
    const description =
      ...
      'A from-scratch, dependency-free HTML5-canvas charting engine for OpenAlgo: 15 chart types, 102 built-in indicators plus your own, 51 drawing tools, on-chart trading, and live data, from a 59 KB base engine.';
    return (
      <>
