...
- **Wheel**: zoom both axes around the cursor, eased over a few frames (`animZoom: false`
  for the single-frame step); `zoomAnchor: 'right'` holds the latest bar still instead.
  `wheelZoomSensitivity` scales the step and defaults to `1`; lower values tame
  high-frequency trackpad bursts and `0` disables wheel zoom without changing
  pinch or keyboard zoom.
