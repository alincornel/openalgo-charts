...
- **Wheel zoom sensitivity**: `wheelZoomSensitivity` multiplies every wheel zoom, on the
  plot and on a price axis, and defaults to `1`; lower values slow it, `0` disables wheel
  zoom without changing wheel pans, pinch or keyboard zoom.
- **Drag the plot**: press and hold to show a grabbing hand, then move in any direction.
  Mouse and pen pan time and price by default and stop immediately on release. Set
...
- **Drag the price axis**: rescale price; **drag the time axis** left to expand bar spacing,
  or right to compress it. The drag emits `zoom` as it goes, and says nothing on
  a gesture that leaves the window where it was.
