...
<Callout type="info">
[Conflation](/docs/data-loading#large-datasets-conflation) drops per-bar colour: a merged
bucket of bars has no single one. It is off by default and only engages when bars fall
below sub-pixel widths, but a chart that turns it on loses the tint at the zoom levels
where it merges.
</Callout>
...
| `lastValueLabelColor` | all | fixed colour for the last-value tag, and for the last-price line on the instrument (default: the last bar's colour) |
...
### A last-price tag that holds still

The instrument's axis tag and its dashed price line take the last bar's own colour, so on
a live feed they flip green to red and back with every tick that crosses the open.
`lastValueLabelColor` pins both to one colour whichever way the bar closed, and picks the
tag's text for contrast against it, so a pale colour reads as well as a dark one:

```ts
chart.addSeries('candlestick', { style: { lastValueLabelColor: '#8892a6' } });
```

It only changes the colouring. `lastValueVisible` and `priceLineVisible` still decide
whether the tag and the line are drawn at all. Set on any other series, it fixes that
series' own axis tag the same way.
