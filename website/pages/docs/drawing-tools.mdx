---
title: Drawing Tools
description: 34 drawing tools, a headless controller, undo/redo, and persistence.
---
...
`openalgo-charts/draw` is a lazy tier (**8.3 KB** Brotli) with 34 tools and a headless
...
**34 built-in tools**, all registered when the tier is imported.

| Group | `id` | Anchors |
|---|---|---|
| Lines | `trend-line`, `ray`, `extended-line`, `arrow` | 2 |
| Horizontal / vertical | `horizontal-line`, `horizontal-ray`, `vertical-line`, `cross-line` | 1 |
| Shapes | `rectangle`, `ellipse`, `circle`, `triangle` | 2 / 2 / 2 / 3 |
| Paths | `polyline` (free-form), `arc`, `curve` | n / 3 / 3 |
...
| Annotation | `text` (1), `path`, `highlighter` (free-form) | 1 / n |

`long-position` and `short-position` report **R:R** and a risk-based position size from
`accountSize` and `risk` in their style bag.
