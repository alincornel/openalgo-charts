---
title: Drawing Tools
description: 43 drawing tools, a headless controller, undo/redo, and persistence.
---
...
`openalgo-charts/draw` is a lazy tier (**11.7 KB** Brotli) with 43 tools and a headless
...
**43 built-in tools**, all registered when the tier is imported.
...
`draw:tool`, `draw:add`, `draw:update`, `draw:remove`, `draw:select` all fire on
`chart.on(...)`.
...
shape-text properties panel exposes. `'outside'` parks the block above the shape and ignores
