...
interface ChartOptions {
  ...
  /**
   * Where indicator legend rows start inside one pane, in media px.
   * Default `{ top: 6, left: 8, paneIndex: 0 }`.
   */
...
It applies to **one** pane — `paneIndex`, default 0 — because that overlay is
nearly always on the price pane. Lower indicator panes are short, so shifting
them by a price-pane offset would push their legend, and its settings / close /
move-pane buttons, off the pane entirely.
</Callout>
