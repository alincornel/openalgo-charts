...
Charts now owns the responsive corner logo. The companion OpenAlgo change removes
its manually attached logo and exposes an accessible link that follows host branding.
Optional watermark controls use the shared Appearance schema and saved chart state;
the background text is off by default. Automatic text follows the current symbol
and interval, and Replay retains its separate marker.
