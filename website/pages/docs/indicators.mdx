---
title: Indicators
description: 19 built-in indicators via chart.addIndicator, plus the registry for your own.
---
...
The 19 built-ins live in the lazy `openalgo-charts/indicators` tier.

```ts
import { createChart } from 'openalgo-charts';
import 'openalgo-charts/indicators';   // registers all 19 built-ins
...
| William VIX FIX | `williams-vix-fix` | pane | `pd`, `bbl`, `mult`, `lb`, `ph`, `pl`, `hp`, `sd` |
...
| `INDICATOR_PLOT_STYLES` | `readonly { label, value }[]` | Chart types a plot can be re-rendered as (line, step, area, histogram, columns). |

```ts
import {
  registeredIndicators, hasIndicator, indicatorDefaults,
} from 'openalgo-charts';
import 'openalgo-charts/indicators';   // registers the 19 built-ins
