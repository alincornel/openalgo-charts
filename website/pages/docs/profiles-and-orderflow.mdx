...
row becomes the **adjacent price** rather than the next price that happened to trade. A
positive quantity against zero opposing volume qualifies at any ratio, so an isolated ask
print flags a buy imbalance more readily. `imbalanceRatio` cannot damp that case — nothing
times zero is still zero — so **`imbalanceThreshold` is the knob** for it: it sets the
quantity a side has to reach before it counts at all.
