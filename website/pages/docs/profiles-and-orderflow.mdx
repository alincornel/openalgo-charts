...
| Row | Value |
| --- | --- |
| `volume` | Sum of bid and ask volume, plus `neutralVol` (volume with no aggressor side) when the bar carries it |
...
| `deltaPct` | Delta as a percentage of the bar's bid plus ask volume |
...
`diagonalImbalances(cells, ratio = 3, rowSize?, threshold = 0, minOpposing = 0)` scans the `FootprintCell[]` of one bar (sorted high
...
`minOpposing` is the floor the opposing quantity is compared **at**, and it exists because
nothing times zero is zero: against an empty row the ratio drops out of the comparison and
any positive print qualifies, at 3 and at a billion alike. That is the honest reading for a
sparse ladder, and it is the wrong one once rows have been *manufactured*. The `Footprint`
primitive passes `1`, so `imbalanceRatio` still means something under `zeroFill`; the
default `0` leaves the comparison as fractional quantities deserve.
...
const ladder = diagonalImbalances(bar.cells, 3, bar.rowSize, 0, 1); // the renderer's rule
...
`stackedImbalances(cells, ratio = 3, minStack = 3, rowSize?, threshold = 0, minOpposing = 0)` builds on `diagonalImbalances` and
