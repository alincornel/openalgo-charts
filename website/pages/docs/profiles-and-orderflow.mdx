...
delta, at an alpha set by how much of the bar's busiest row it carries, and drops the
saturated highlight so the two signals do not fight. `pocOutline` takes a colour and strokes
the highest-volume row, for when the `showPoc` tick in the margin gets lost in a dense ladder.
