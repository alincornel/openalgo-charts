...
`enabled` is optional and its absence is information. Borders and wicks have
`borderVisible` and `wickVisible` behind them, so those rows get a switch that does
something. A candle **body** has no such flag, because a candle with no body is not a
candle, so the Body row ships with two swatches and an empty switch column rather than a
checkbox that would be a lie.
