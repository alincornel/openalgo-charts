...
import { ProfileOverviewScreenshot, ProfileThemeGallery } from '../../components/ProfileShowcase'
...
**Since 2.1.0**, compact pixel letters, individual split/unsplit controls and
open/latest-price markers are available. Select `blockDisplay: 'compact'` to use
the compact renderer; the library default remains `'auto'`.
[Open the interactive demo](/docs/market-profile-examples) to change themes and split a day.

<ProfileOverviewScreenshot />
...
from the left. The synthetic bars use 0.1 ticks and two ticks per row (0.2-point
rows); the candle series supplies the time axis and is hidden for clarity:
...
return chart;`} caption="Composite TPO with compact letters packed left, open/latest-price markers, POC (gold) and a faint value-area fill." />
...
chart.timeScale.fitContent(bars.length);
return chart;`} caption="Compact day sessions: a separate profile per calendar day, on the default Asia/Kolkata, each with its own POC, value area, and initial-balance bracket." />
...
sub-profile. This example deliberately uses `'blocks'` to compare heat colouring
with the compact letter examples above:
...
These browser captures use the same synthetic session and themes as the
[interactive demo](/docs/market-profile-examples). They are **zoomed close-ups**:
18 CSS pixels per row and 16-pixel letters at double resolution. Compact mode
uses regular text at this zoom; the overview above shows its compressed pixel
glyphs. Click a screenshot to inspect its original resolution.

<ProfileThemeGallery />

See [packed and split views](/docs/market-profile-examples#packed-and-split-views)
for the same session before and after splitting.
...
| Letters are too small | Too many price rows for the chart height | Use `blockDisplay: 'compact'` or zoom vertically; increase `rowTicks` only when you intend to aggregate more ticks per row |
