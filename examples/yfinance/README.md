...
| **Volume on and off** | An eye on the volume legend row, and a row in the right-click menu. Volume rides an overlay scale inside the price pane rather than being a study you re-add from a list, so it hides and shows, and there is no delete beside the eye that would only be a second spelling of hide. The choice survives a reload and a chart-type switch. |
| **Tooltips** | Icon-only controls carry a real tooltip rather than a browser `title`: the rail names its tool and says double-clicking keeps it armed, which is wired here and not just claimed. Labels move to `aria-label`, the tip never takes pointer events, and it flips back inside the window at an edge. |
