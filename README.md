...
Professional interactive charts, 102 built-in indicators plus your own custom ones, 85 drawing tools, order flow, market replay, linked chart grids, on-chart trading, vector SVG export and an optional WebGL2 backend. Eight lazy-loaded tiers, zero runtime dependencies, 76.52 KB Brotli for the base engine, and a one-call widget tier that adds the toolbar, drawing rail, dialogs and shortcuts.
...
[![bundle](https://img.shields.io/badge/brotli-76.52%20KB%20base%20%C2%B7%20215.04%20KB%20all%20tiers-brightgreen.svg)](#size-budget)
...
<script type="module">
  import { createChart } from 'https://unpkg.com/openalgo-charts@2.3.0/dist/openalgo-charts.mjs';
...
|---|---|---|
| `openalgo-charts` | Engine, 13 chart types, panes &amp; scales, primitives, registries, chart state, chart linking, bar cache, interval registry, trading overlay, SVG export, render backend port, OpenAlgo feeds | 76.52 KB |
...
Everything together is **215.04 KB Brotli**; a widget terminal (base + draw + indicators + widget, what one `createWidget` call loads) is 181.66 KB. Figures are measured from the 2.3.0 release build. The trade tier is listed as its delta over the base, so loading base + trade costs 84.13 KB.
...
| Bundle | Limit | Actual |
|---|---|---|
| Base engine | 77 KB | 76.52 KB |
| Base + trade | 85 KB | 84.13 KB |
...
| Widget terminal (base + draw + indicators + widget) | 183 KB | 181.66 KB |
| **Everything** | 218 KB | 215.04 KB |
...
npm test           # unit tests (vitest) - 5182 across 208 files
...
Version **2.3.0**. All engine build phases are implemented. Upgrading a 1.9.x host: [Migrating to 2.0](./docs/migrating-to-2.md).
