...
Professional interactive charts, 102 built-in indicators plus your own custom ones, 85 drawing tools, order flow, market replay, linked chart grids, on-chart trading, vector SVG export and an optional WebGL2 backend. Eight lazy-loaded tiers, zero runtime dependencies, 76.39 KB Brotli for the base engine, and a one-call widget tier that adds the toolbar, drawing rail, dialogs and shortcuts.
...
[![bundle](https://img.shields.io/badge/brotli-76.39%20KB%20base%20%C2%B7%20213.06%20KB%20all%20tiers-brightgreen.svg)](#size-budget)
[![tests](https://img.shields.io/badge/tests-5150%20passing-brightgreen.svg)](#develop)
...
and the [2.2.2 changelog](./CHANGELOG.md#222).
...
<script type="module">
  import { createChart } from 'https://unpkg.com/openalgo-charts@2.2.2/dist/openalgo-charts.mjs';
...
|---|---|---|
| `openalgo-charts` | Engine, 13 chart types, panes &amp; scales, primitives, registries, chart state, chart linking, bar cache, interval registry, trading overlay, SVG export, render backend port, OpenAlgo feeds | 76.39 KB |
...
Everything together is **213.06 KB Brotli**; a widget terminal (base + draw + indicators + widget, what one `createWidget` call loads) is 181.46 KB. Figures are measured from the 2.2.2 release build. The trade tier is listed as its delta over the base, so loading base + trade costs 84.00 KB.
...
| Bundle | Limit | Actual |
|---|---|---|
| Base engine | 77 KB | 76.39 KB |
| Base + trade | 85 KB | 84.00 KB |
...
| Widget terminal (base + draw + indicators + widget) | 183 KB | 181.46 KB |
| **Everything** | 215 KB | 213.06 KB |
...
Version **2.2.2**. All engine build phases are implemented. Upgrading a 1.9.x host: [Migrating to 2.0](./docs/migrating-to-2.md).
