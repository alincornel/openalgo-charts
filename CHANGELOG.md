...
- **Upstream 2.0.2** (`marketcalls/openalgo-charts`, 27 commits since 1.9.2):
  the `IRenderBackend` port with a pixel-identical Canvas2D backend and a lazy
  WebGL2 tier, the breaking 2.0 drawing model (z-order, multi-select, settings
  schema, per-level fibs, two-click position tool), the `openalgo-charts/widget`
  tier, synchronous SVG export, the eased wheel zoom with `zoomAnchor`, the
  `doubleClick` option, and an ESLint tier ACL that fails the build on a
  base-to-tier import. Everything the fork carries survives it:

  - **`wheelZoomSensitivity` re-applied on the glide.** Upstream rewrote
    `_onWheel` to ease a wheel step over a few frames in LOG space, so the
    multiplier is now applied there: `1.1 ** s` is `s * ln(1.1)`, which is
    bit-for-bit the shipped step at the default `1` and damps or disables every
    event of a trackpad burst exactly as before. The `deltaY === 0` guard stays;
    without it upstream's sign test reads a zero delta as a zoom OUT.
  - **The touch crosshair carries the 2.0 pointer payload.** `_updateCursor`
    gained a `source: PointerEvent` argument for the modifier flags, the
    pressure and the coalesced samples a `crosshair:move` now reports. The three
    fork paths that summon or steer the crosshair — the long press, the adopted
    pointer and the steer — pass the event that caused them, so a summoned
    crosshair reports the press that summoned it rather than nothing.
  - **Primitives are untouched by the backend port.** The GPU backend adds no
    canvas: it rasterises only the series pass onto a shared surface and blits
    it into the base canvas at `endFrame`, between the grid and the normal-layer
    primitives. Every primitive the fork ships or extends — the price-line
    pills, the footprint ladder, the volume profile — keeps drawing on the 2D
    context the pane hands it, identically under `canvas2d` and `webgl2`.

- Budgets re-measured on the merged tree rather than carried over: base engine
  68.90 kB against 69.3, base + trade 76.59 against 77, profile 11.85 against
  11.9, widget terminal 157.54 against 158, everything 186.12 against 186.6.
...
- Budgets re-measured on the merged tree: base engine 68.90 -> 76.03 kB (limit
  69.3 -> 76.2), base + trade 76.59 -> 83.72 (77 -> 83.9), profile tier
  13.12 -> 16.86 (13.2 -> 16.9), widget terminal 157.54 -> 168.69 (158 ->
...
- **Upstream 2.4.0** merged on top (4 commits): coverage for ported studies —
  `securitySeries` for higher-timeframe reads, `IndicatorPlot.offset` and
  `SeriesStyle.barOffset`, the recompute guard and `IndicatorInputError`, alert
  messages built from the firing bar, `cross`/`xcross` markers and
  `paneTop`/`paneBottom` positions, price-pane fills, per-bar wick and border
  colours on both candle paths, tooltips on label and box drawings, interval and
  time inputs, and a bars provider for combining a benchmark with the chart's
  bars. One conflict in `src/`: `_plotStyle` keeps both upstream's `barOffset`
  and the fork's reserved `lastValueVisible` setting, read after the declared
  style so a caller can still turn off a tag the descriptor asked for.

- Budgets re-measured on the merged tree: base engine 81.20 kB (limit 81.6),
  base + trade 88.89 (89.3), widget terminal 187.27 (187.7), everything 222.83
  (223.3), against upstream's 78.07 / 85.68 / 184.14 / 217.54. Chart-only shake
  51.92 KiB (52.1): upstream's own build went 49.01 -> 49.86 on 2.4.0, the fork
  51.06 -> 51.92, so the growth is upstream's alone. 5529 tests across 236 files.

- **Upstream 2.4.5 - 2.4.8** merged on top: CSV export, the `workspace` tier,
  alerts with draggable lines, price-anchored study markers, legend source
  buttons, the primitive drag lifecycle (`drag:start`, `drag:cancel` carrying
  `{ id, paneIndex, reason: 'pointercancel' | 'pinch' | 'escape' }`, opt-in
  Escape via `cancelOnEscape`) and mouse/pen panning without inertia. Conflicts
  in `src/core/chart.ts` only. The fork's cancel semantics now ride on
  upstream's event rather than a second one: `_beginPinch` and `pointercancel`
  emit upstream's `drag:cancel` once and additionally call `onDragCancel`; a
  host that registered `onDragCancel` does NOT get the legacy `drag:end` /
  `onDragEnd` after a `pointercancel` (upstream sends it to hosts that have no
  cancel callback, and still does). A cancelled pan drops its grabbing cursor.
  The adopted crosshair, the tap-on-empty-plot dismissal and pinch clearing the
  crosshair pointer are unchanged. Upstream's new `getCachedBars` assertion for
  a future `storedAt` is replaced in `bar-cache-resilience.test.ts` by the
  fork's clock-steps-backwards model (closed bars kept, tail refetched once).
- Budgets re-measured: base engine 96.03 kB (limit 96.4), base + trade 104.12
